import { ActivityIndicator, BackHandler, Dimensions, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
const CategoriesScreen = ({ route }) => {
  ...
  useEffect(() => {
    setLoading(true)
    if (selected) {
      axios.get(`/rest/category_sub/${selected}`)
      .then(res => {
        setLoading(false)
    ...
    <View style={styles.cont}>
      <View style={{ marginHorizontal: 20, marginVertical: 10 }}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginVertical: 10 }}>
          {
            categories.length > 0 && categories.map((item, i) => <CategoriesCard data={item} key={item.cid} select={e => setSelected(e)} />)
          }
        </View>
      </View>


      <View style={{ borderTopWidth: 2, borderColor: Colors.primary, marginHorizontal: 20, marginVertical: 10 }}>
        <Icon name="caretdown" size={19} color={Colors.primary} style={{ alignSelf: 'center' }} />
      </View>
      {selected && <View style={{ marginHorizontal: 20, marginVertical: 10 }}>
        <Text style={{ ...styles.text, fontWeight: 'bold' }}>Sub Categories</Text>
        <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginVertical: 10, marginHorizontal: 20 }}>
          {
            subCategories.length>0 ? subCategories.map((item, i) => <CategoriesCard data={item} key={i} sub={true} />)
