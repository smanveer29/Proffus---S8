...
const LoginScreen = () => {
  ...
  const login = () => {
    setLoading(true);
    auth()
      .verifyPhoneNumber('+91' + data.phoneNumber)
      .then(res => {
        setLoading(false);
        console.log(res,'login firebase response')
        if (res) {
          setConfirm(res);
          setUser(data);
        }
  ...
  return (
    <ImageBackground
      source={require('../../Assets/Images/bombaybg.png')}
      style={styles.cont}>
      {confirm === null ? (
        <ScrollView>
          <KeyboardAvoidingView style={{margin: 20}}>
            ...
            <AuthInputs
              ...
              left={
                <TextInput.Icon
                  name={() => <Icon name={'phone'} size={16} color="#000" />}
                />
              }
            />
          ...
          <View style={{...dfrcc, marginBottom: 30}}>
            ...
            <Text style={{...p, textTransform: 'uppercase', letterSpacing: 2}}>
              {' '}
              or new here?{' '}
            </Text>
            <Text
